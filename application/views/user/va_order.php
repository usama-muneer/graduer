            ...
            <?php
              ...
              if($data){
                  ...
                  foreach($data as $row){
                    ...
                    WHERE userprofile.user_id='. $buyer_id . ' AND users.user_id='. $buyer_id . ' AND orderrequirements.order_id=' . $order_id . ' AND orders.order_id =' . $order_id ;
                    $data = $this->db->query($sql)->result_array();
                    if($data){
              ...
              <tr>
                ...
                <td >
                  <?php echo '<img src="'.base_url().$row1['picture'].'" alt="'. $freelancer_id .'" class="img-va-order text-center" />'; echo "<br/>"; echo '<small>' . $row1['username'] . '</small>'; ?>
                </td>
                <td>
  ...
  <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
    <table class="table">
      ...
      <tbody>
        <?php
          $user_id = $this->session->userdata('user_id');
          $sql = 'SELECT * FROM orders WHERE order_status IN ("Completed") AND freelancer_id ='. $user_id ;
          $data = $this->db->query($sql)->result_array();
          if($data){
              $val = 1;
              foreach($data as $row){
