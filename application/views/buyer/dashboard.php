<?php
  ...
  //FETCH ALL ACTIVE ORDER DATA INFO
	$sql4 = 'select count(order_id) as torders from orders where order_status = "Active" AND buyer_id=' . $user_id;
  ...
  //FETCH ALL Active GIGS ORDER DATA INFO
	$sql5 = 'select count(brequest_id) as trequests from buyerrequest where brequest_status = "Active" AND buyer_id=' . $user_id;
	$data5 = $this->db->query($sql5)->result_array();
	if($data5){
		foreach($data5 as $row):
		  $trequests = $row['trequests'];
		  endforeach;
		  }
	else{
		$not = "No data found.";
  ...
  //FETCH ALL Active GIGS ORDER DATA INFO
	$spendings_sql = 'SELECT sum(order_price) as tspending FROM orders WHERE  buyer_id=' . $this->session->userdata('buyer_id') .' AND approve_date <> ""';
	$spendings_data = $this->db->query($spendings_sql)->result_array();
	if($spendings_data){
		foreach($spendings_data as $row):
		  $tspending = $row['tspending'];
		endforeach;
	}
  else {
    $earnings = "$ 0";
      ...
      <div class="col-lg-3 col-sm-12 col-12 text-center detail-part">
        <div class="row image-part m-0">
          ...
          <div class="col-lg-12 col-sm-12 col-12 information-button">
            <a href="<?php echo base_url(); ?>bprofilec/view_profile" class="btn btn-success w-75">View</a>
          </div>
                ...
                <div class="col-xl-4 col-sm-6 py-2">
                    <div class="card bg-success text-white h-100">
                        <div class="card-body bg-success">
                            ...
                            <h1 class="display-4"><?php echo $torders; ?></h1>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-xl-4 col-sm-6 py-2">
                    <div class="card text-white bg-warning h-100">
                        <div class="card-body bg-warning">
                            ...
                            <h6 class="text-uppercase padding10">Active Requests</h6>
                            <h1 class="display-4"><?php echo $trequests; ?></h1>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-xl-4 col-sm-6 py-2">
                    <div class="card text-white bg-info h-100">
                        <div class="card-body bg-info">
                            ...
                            <h6 class="text-uppercase padding10">Total Spending</h6>
                            <h1 class="display-4">$ <?php echo $tspending; ?></h1>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
              <table class="table">
                ...
                <tbody>
                  <?php
                    $user_id = $this->session->userdata('buyer_id');
                    $sql = 'SELECT * FROM orders WHERE order_status IN ("Active", "Delivered") AND buyer_id ='. $user_id ;
                    ...
                    if($data){
                        ...
                        foreach($data as $row){
                          ...
                          WHERE userprofile.user_id='. $freelancer_id . ' AND users.user_id='. $freelancer_id . ' AND orders.order_id =' . $order_id ;
                          $data = $this->db->query($sql)->result_array();
                          if($data){
                            foreach($data as $row){
                              $picture = $row['picture'];
